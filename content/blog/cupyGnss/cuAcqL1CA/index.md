...
Regardless of the sample rate of the recording, the script will resample to a canonical 4.096 Msps. That sample rate is high enough for the PRN code and a power of 2, required for FFT efficiency. At the beginning I tried to port the original GNSS-DSP-tools `resample` to CuPy, achieving ~10 run times. Diving deeper into CuPy's documentation I found that the Nvidia team already provide `cupyx.scipy.signal.resample`. Using it yields almost identical results and is more than twice as fast, around ~4 ms.
...
In my use case, where I have a Laptop RTX 4060, a runtime of ~5 ms is achieved. Amazing in my humble opinion. Let us not remember that these 5 ms include:
...
Performance-wise, I am averaging a bit less of **~7 ms per PRN**. Sub 10 milliseconds per PRN is great, specially taking into account that a real receiver needn't have all PRNs under acquisition at the same time, and that 80 ms of signal have been accumulated.
...
```bash
prn   1 doppler -3200.0 metric  1.55 code_offset  142.6
prn   2 doppler  1800.0 metric  3.22 code_offset  758.8
prn   3 doppler -1200.0 metric  1.65 code_offset 1004.0
prn   4 doppler  6600.0 metric  1.40 code_offset  521.7
prn   5 doppler   800.0 metric  1.61 code_offset  964.8
prn   6 doppler  1800.0 metric  1.54 code_offset  153.4
prn   7 doppler -2400.0 metric  1.35 code_offset  490.8
prn   8 doppler  3600.0 metric  1.52 code_offset  147.9
prn   9 doppler  4800.0 metric  1.48 code_offset  376.6
prn  10 doppler -5200.0 metric  1.54 code_offset  544.0
prn  11 doppler  2400.0 metric  7.29 code_offset  185.8
prn  12 doppler  -200.0 metric  8.01 code_offset  151.6
prn  13 doppler   800.0 metric  1.63 code_offset  881.4
prn  14 doppler -1200.0 metric  1.51 code_offset  123.1
prn  15 doppler -4200.0 metric  1.53 code_offset  813.5
prn  16 doppler  4600.0 metric  1.40 code_offset  253.8
prn  17 doppler -4400.0 metric  1.40 code_offset  624.1
prn  18 doppler  1800.0 metric  1.51 code_offset  860.9
prn  19 doppler -5000.0 metric  1.48 code_offset  146.4
prn  20 doppler  2800.0 metric  1.54 code_offset  169.6
prn  21 doppler -5200.0 metric  1.57 code_offset  438.3
prn  22 doppler  2600.0 metric  5.36 code_offset  465.0
prn  23 doppler -7000.0 metric  1.53 code_offset  423.3
prn  24 doppler  2600.0 metric  1.53 code_offset  508.3
prn  25 doppler  1600.0 metric  7.10 code_offset  425.1
prn  26 doppler -2000.0 metric  1.29 code_offset  302.0
prn  27 doppler -1200.0 metric  1.46 code_offset  404.9
prn  28 doppler  6800.0 metric  1.52 code_offset  712.8
prn  29 doppler -3200.0 metric  1.50 code_offset  470.0
prn  30 doppler  6800.0 metric  1.45 code_offset  708.3
prn  31 doppler  5200.0 metric  6.89 code_offset  559.7
prn  32 doppler  1000.0 metric  7.17 code_offset  680.3
```
...
On my PC, running Peter Monta's `acquire-gps-l1.py` for all 32 PRNs takes around **~3.5 seconds**. Note that he uses the multiprocessing library to make use of all cores in your computer, so you mileage may vary. Running my CUDA-accelerated version of his script takes **~230 milliseconds** on the RTX 4060. In other words, if a subset of PRNs were to be acquired every acquisition iteration (say 4 PRNs per run e.g. [1,2,3,4], then [5,6,7,8] and so on) **real-time could be achieved**.

> Note: You will see that, in order to get representative timings, I have timed iterations excluding the first one. This is because first iteration will take more time due to modules loading into the GPU.

## Next steps

1. Batching the PRN search: Instead of one call to `search` per PRN, it would be interesting to try and batch multiple PRN searches and do them in one call. Perhaps one call to cuFFT with a large array is faster than several calls with smaller arrays. It needs to be tested, I'm not an expert on the optimizations of cuFFT.

2. Dive deeper in the retrieval of data from GPU to CPU, and moving samples from CPU to GPU. There are a lot of concepts regarding fixed memory pools for the DMA to move that I still need to wrap my head around.
